...
// use pluto_token::common::OrderBy;
use crate::gov::{
    Cw20HookMsg, ExecuteMsg, ConfigResponse, InstantiateMsg,  PollResponse,
    PollStatus, PollsResponse, QueryMsg, StateResponse, VoteOption, VoterInfo, VotersResponse,
    VotersResponseItem, OrderBy
};
...
pub fn receive_cw20(
    ...
    if config.pluto_token != deps.api.addr_canonicalize(info.sender.as_str())? {
...
pub fn update_config(
    ...
    config_store(deps.storage).update(|mut config| {
        ...
        if let Some(pluto_token) = pluto_token {
            config.pluto_token = api.addr_canonicalize(&pluto_token)?;
        }
...
pub fn end_poll(deps: DepsMut, env: Env, poll_id: u64) -> Result<Response, ContractError> {
    ...
    let (quorum, staked_weight) =
    if state.total_share.u128() == 0 {
        (Decimal::zero(), Uint128::zero())
    } else if let Some(staked_amount) = a_poll.staked_amount { // staked_amount = all staked tokens
        (
...
fn query_config(deps: Deps) -> Result<ConfigResponse, ContractError> {
    ...
    Ok(ConfigResponse {
        ...
        pluto_token: deps.api.addr_humanize(&config.pluto_token)?.to_string(),
